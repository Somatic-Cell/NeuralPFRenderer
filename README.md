...
```
下に置かれる． \
``` ptxes ``` フォルダには，CUDA / OptiX 関係のコンパイル結果 (```.ptx```, ```.optixir```) が格納される．ビルド時のログは ``` {コンパイルされたファイル名}.log``` として保存されているため，コンパイルエラーが発生したときには該当するファイルを確認すること．\
また，``` Debug ``` or ``` Release ``` フォルダには，実行ファイルを含めた種々のファイルが生成される．
...
実行の際は，``` /build/bin/{Release | Debug}  ``` 下にある実行ファイル ``` AtmosphericRT ``` を実行すればよい．引数は

```
.AtmosphericRT {使用したいシーンファイルの /scene フォルダ下からの相対パス}.json 
```
のようにすると，シーンファイルで定義された種々のパラメータが読み込まれ，実行される．
シーン内の要素（カメラパラメータ，スペクトラルレンダリングの ON / OFF, 使用するオブジェクト，...） だけを変更したい場合は，実行前にいちいちビルドし直す必要はない．

ビルドファイルと同様，プロジェクトのルートディレクトリに，実行のための ``` execute.bat ``` ファイルを用意している．基本的にはプロジェクトのルートディレクトリにいる状態で，バッチファイルを実行するかたちでビルドや実行を行えばいい．
...
マウス入力を受け付ける．
* 左ドラッグ：カメラ位置を中心とした回転
* ホイールのドラッグ：スクリーンに対して平行な平行移動
* 右ドラッグ：スクリーンに対して垂直な平行移動（前後移動）
